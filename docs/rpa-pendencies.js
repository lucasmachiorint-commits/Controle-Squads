  ...
  const RpaPendenciesModule = {
          ...
          <div class="modal-backdrop hidden" id="modal-rpa-edit" style="z-index: 99999; backdrop-filter: blur(8px); display: none;">
            <div class="modal-content relative" style="max-width: 540px; background: #0f172a; border: 1px solid #1f2937; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.75); border-radius: 16px; padding: 24px; overflow: visible;">
              ...
              <form onsubmit="RpaPendenciesModule.savePendency(event)">
                ...
                <!-- 1. ROBÔ(S) AFETADO(S): FRAME UNIFICADO COM TAGS INTERNAS QUE EXPANDE DINAMICAMENTE -->
                <div class="form-group mb-4">
                  <label class="text-xs font-semibold text-gray-300 mb-1.5 block">Robô(s) Afetado(s) <span class="required-asterisk">*</span></label>
                  
                  <div class="w-full bg-[#111827] border border-[#1f2937] focus-within:border-emerald-500 rounded-lg p-2 flex flex-wrap gap-1.5 items-center min-h-[42px] transition-all">
                    <div id="rpa-robot-pills-container" class="flex flex-wrap gap-1.5 items-center">
                      <!-- Tags geradas dentro do quadro -->
                    </div>

                    <select id="rpa-robot-adder-select" 
                            class="bg-transparent text-gray-300 text-xs border-none outline-none cursor-pointer flex-1 min-w-[150px] py-1 focus:ring-0"
                            onchange="RpaPendenciesModule.addRobot(this.value); this.value = '';">
                      <option value="" class="bg-[#111827] text-gray-400">+ Selecionar Robô...</option>
                ...
                <!-- 2. RESPONSÁVEL(IS): FRAME UNIFICADO COM TAGS INTERNAS QUE EXPANDE DINAMICAMENTE -->
                <div class="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    <label class="text-xs font-semibold text-gray-300 mb-1.5 block">Responsável <span class="required-asterisk">*</span></label>
                    
                    <div class="w-full bg-[#111827] border border-[#1f2937] focus-within:border-emerald-500 rounded-lg p-2 flex flex-wrap gap-1.5 items-center min-h-[42px] transition-all">
                      <div id="rpa-resp-pills-container" class="flex flex-wrap gap-1.5 items-center">
                        <!-- Tags geradas dentro do quadro -->
                      </div>

                      <select id="rpa-resp-adder-select" 
                              class="bg-transparent text-gray-300 text-xs border-none outline-none cursor-pointer flex-1 min-w-[130px] py-1 focus:ring-0"
                              onchange="RpaPendenciesModule.addResponsible(this.value); this.value = '';">
                        <option value="" class="bg-[#111827] text-gray-400">+ Responsável...</option>
    ...
    renderRobotPills() {
      ...
      container.innerHTML = this.selectedRobots.map(botName => {
        ...
        return `
          <span class="inline-flex items-center gap-1.5 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-[11px] px-2 py-0.5 rounded-md font-medium shadow-sm transition-all">
            <i class="fa-solid fa-robot text-[10px]"></i>
            <span>${botName}</span>
            <button type="button" onclick="RpaPendenciesModule.removeRobot('${safeName}')" class="text-emerald-400/80 hover:text-emerald-200 ml-0.5 cursor-pointer" title="Remover">
    ...
    renderRespPills() {
      ...
      container.innerHTML = this.selectedResponsibles.map(rName => {
        ...
        return `
          <span class="inline-flex items-center gap-1.5 bg-indigo-500/15 border border-indigo-500/30 text-indigo-300 text-[11px] px-2 py-0.5 rounded-md font-medium shadow-sm transition-all">
            <i class="fa-solid fa-user text-[10px]"></i>
            <span>${rName}</span>
            <button type="button" onclick="RpaPendenciesModule.removeResponsible('${safeName}')" class="text-indigo-300/80 hover:text-indigo-200 ml-0.5 cursor-pointer" title="Remover">
              <i class="fa-solid fa-xmark text-[10px]"></i>
            </button>
          </span>
        `;
